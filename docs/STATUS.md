...
- **Recording channels renamed to say what they record**:
  `#coaching-recordings`, `#setting-recordings`, `#sales-recordings`. Merging
  the brand categories made three channels called `call-recordings` visible to
  the same client at once; a test now enforces that no two channel names
  collide anywhere in the server. THE CALLED has no recordings channel —
  Warrior Huddles and bible study aren't recorded.
