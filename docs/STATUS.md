...
| ~~3~~ | ~~Friday Weekly Check-in reminder~~ | ❌ **Removed 2026-09-15** | Removed at the CSM's request — clients are prompted in their 1:1s instead. It ran live from 2026-09-09 to 2026-09-12, posting into each client's private channel every Friday at noon ET. The send path, its tests, `npm run weekly-reminder-dry-run` and the `WEEKLY_REMINDER_*` / `WEEKLY_CHECKIN_FORM_URL` / `DISCORD_TEST_CHANNEL_ID` variables all came out together; `git revert` the removal commit restores it. **No automation sends a client anything on a schedule any more** — everything left posts to a staff channel. The `Discord ID` field and the `Skip Weekly Reminder` opt-out both outlive it: Discord ID links a client to tier sync and onboarding, and `Skip Weekly Reminder` now means "not expected to check in at all" to the Saturday report (#6). |
