        ...
        {series.map((s, si) => (
          <g key={s.key}>
            ...
            {runs(s.points).map((run, ri) => (
              <polyline
                key={ri}
                className={`chart-line ${s.person ? `person-${s.person}` : `series-${si + 1}`}`}
            ...
            {s.points.map((p, i) =>
              p.y === null ? null : (
                <circle
                  ...
                  cy={y(p.y)}
                  r={hover === i ? 5 : 3.5}
                  className={`chart-dot ${s.person ? `person-${s.person}` : `series-${si + 1}`}`}
                />
              )
            )}
          </g>
      ...
      <div className="chart-legend">
        {series.map((s, si) => (
          <span key={s.key} className="chart-legend-item">
            <span className={`chart-swatch ${s.person ? `person-${s.person}` : `series-${si + 1}`}`} />
            {s.label}
            {hover !== null && (
              <strong>
